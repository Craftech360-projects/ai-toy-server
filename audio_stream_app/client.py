...
def setup_mqtt_client():
    ...
    for attempt in range(max_retries):
        try:
            print(f"[MQTT Client] Attempting to connect (Attempt {attempt + 1}/{max_retries})...")
            # Use connect_async for non-blocking connection attempt
            mqtt_client_client.connect(MQTT_BROKER, MQTT_PORT, keepalive=60)
            mqtt_client_client.loop_start() # Start network loop regardless of immediate connect success
            print("[MQTT Client] connect() called, loop started. Waiting for connection...")

            # Wait for connection confirmation via on_connect or timeout
            connect_timeout = retry_delay * 1.5 # Give slightly longer than retry delay
            start_time = time.time()
            while not mqtt_client_client.is_connected() and time.time() - start_time < connect_timeout:
                time.sleep(0.1)

            if mqtt_client_client.is_connected():
                 print("[MQTT Client] Connection successful.")
                 return # Success

            print(f"[MQTT Client] Connection attempt {attempt + 1} timed out or failed (check on_connect).")
            mqtt_client_client.loop_stop() # Stop loop for retry

        except Exception as e:
            print(f"[MQTT Client] Error during connection attempt {attempt + 1}: {e}")
            # Ensure loop is stopped if connect raised an exception before loop_start
            if mqtt_client_client.is_connected(): # Should not happen if exception occurred during connect
                 mqtt_client_client.loop_stop()


        if attempt < max_retries - 1:
            print(f"[MQTT Client] Retrying connection in {retry_delay} seconds...")
            time.sleep(retry_delay)
            retry_delay *= 2 # Exponential backoff
        else:
            print("[MQTT Client] Max connection retries reached. Failed to connect.")
            mqtt_client_client = None # Ensure client is None if setup fails
            break # Exit loop
